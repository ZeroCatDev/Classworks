...
export const useAccountStore = defineStore('account', () => {
  ...
  const token = ref(
    localStorage.getItem('auth_token') || localStorage.getItem('auth_access_token') || null,
  )
  ...
  const userProviderDisplay = computed(
    () =>
      profile.value?.providerInfo?.displayName ||
      profile.value?.providerInfo?.name ||
      providerName.value ||
      profile.value?.provider ||
      '',
  )
  const userProviderColor = computed(
    () => profile.value?.providerInfo?.color || providerColor.value || '',
  )
  ...
  const setToken = (newToken) => {
    ...
    if (newToken) {
      ...
      if (accessExpiresAt.value)
        localStorage.setItem('auth_access_exp', String(accessExpiresAt.value))
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (p) {
        providerName.value =
          p.displayName || p.name || profile.value?.provider || providerName.value

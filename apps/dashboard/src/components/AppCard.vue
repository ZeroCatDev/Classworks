...
const fetchApp = async () => {
  try {
    const response = await fetch(
      `https://api.zcservice.houlang.cloud/oauth/applications/${props.appId}`,
    )
...
<template>
  <!-- 卡片视图 -->
  <Card
    :class="cn('app-card cursor-pointer hover:shadow-lg transition-shadow', props.class)"
  ...
  <Dialog v-model:open="showDialog">
    <DialogContent class="max-w-3xl max-h-[85vh] overflow-y-auto">
      ...
      <div v-else-if="!loading && app?.homepage_url" class="mt-6 text-center text-muted-foreground">
        无法加载 README 文件
      </div>
    </DialogContent>
  </Dialog>

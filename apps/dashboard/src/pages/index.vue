      ...
      if (!appInfoCache.value[token.appId]) {
        try {
          const appResponse = await fetch(
            `https://api.zcservice.houlang.cloud/oauth/applications/${token.appId}`,
          )
          if (appResponse.ok) {
...
const authorizeApp = async () => {
  ...
  try {
    ...
    // 调用授权接口
    await apiClient.authorizeApp(appIdToAuthorize.value, deviceUuid.value, options)
...
const bindCurrentDevice = async () => {
  ...
  try {
    ...
    if (error.message.includes('设备不存在')) {
      try {
        await apiClient.registerDevice(deviceUuid.value, deviceInfo.value?.deviceName || null)
        await apiClient.bindDeviceToAccount(deviceUuid.value)
        await loadDeviceInfo()
  ...
  <div class="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      ...
      <div class="container mx-auto px-6 py-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1
                class="text-2xl font-bold bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text"
              >
                Classworks KV
              </h1>
          ...
          <div class="flex items-center gap-2">
            ...
            <template v-if="accountStore.isAuthenticated">
              <DropdownMenu v-model:open="showUserMenu" class="z-50">
                <template #trigger="{ toggle, open }">
                  <Button class="flex items-center gap-2" size="sm" variant="ghost" @click="toggle">
                    <img
                    ...
                    <span
                      v-if="accountStore.profile?.providerInfo"
                      :style="{
                ...
                <DropdownItem
                  :href="accountStore.profile.providerInfo.website"
                  :style="{
                    backgroundColor: (accountStore.profile.providerInfo.color || '#999') + '22',
                    color: accountStore.profile.providerInfo.color || 'inherit',
                    border: `1px solid ${accountStore.profile.providerInfo.color || '#999'}55`,
            ...
            <Button v-else size="sm" variant="outline" @click="showLoginDialog = true">
              <User class="h-4 w-4 mr-2" />
              登录
            </Button>

            <Button :disabled="isLoading" size="icon" variant="outline" @click="loadTokens">
    ...
    <div class="container mx-auto px-6 py-8 max-w-7xl">
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-yellow-700 dark:text-yellow-300 mb-3">
                您的命名空间当前使用设备
                UUID，建议修改为更有意义的名称（如班级名、房间号等），方便自动授权时识别。
              </p>
              <Button
        ...
        <CardHeader class="pb-4">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <Button size="sm" variant="outline" @click="$router.push('/auto-auth-management')">
          ...
          <div class="space-y-4">
            ...
            <div class="relative group">
              ...
              <div class="relative">
                ...
                <div
                  class="flex items-center gap-2 p-4 bg-gradient-to-r from-muted/80 to-muted/60 rounded-lg border"
                >
                  ...
                  >
                    <CheckCircle2
                      v-if="copied === 'namespace'"
                      class="h-4 w-4 text-green-500 animate-in zoom-in-50"
                    />
      ...
      <div v-else class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <div v-for="group in groupedTokens" :key="group.appId" class="space-y-4">
      ...
      <Dialog v-model:open="showAuthorizeDialog">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription> 为应用生成新的访问令牌 </DialogDescription>
          </DialogHeader>
          <div class="space-y-4 py-4">
            <div class="space-y-2">
              <Label for="appId">应用 ID</Label>
              <Input id="appId" v-model="appIdToAuthorize" placeholder="输入应用 ID" />
            </div>
            <div class="space-y-2">
              <Label for="note">备注（可选）</Label>
              <Input id="note" v-model="authNote" placeholder="为此授权添加备注" />
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" @click="showAuthorizeDialog = false"> 取消 </Button>
            <Button @click="authorizeApp"> 授权 </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog v-model:open="showRevokeDialog">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription> 确定要撤销此令牌的授权吗？此操作无法撤销。 </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" @click="revokeToken"> 确认撤销 </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog v-model:open="showTokenDialog">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription> 查看并对该令牌执行操作 </DialogDescription>
          </DialogHeader>
          <div v-if="selectedToken" class="space-y-3 py-2">
            ...
            <div class="flex items-center gap-2 text-sm">
              ...
              <code class="text-xs font-mono break-all"
                >{{ selectedToken.token.slice(0, 8) }}...</code
              >
              <Button
              ...
              >
                <CheckCircle2
                  v-if="copied === selectedToken.token"

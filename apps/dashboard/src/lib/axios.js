...
import { DEFAULT_LOCAL_SERVER, HEADER_SITE_KEY, HEADER_DEVICE_UUID } from '@classworks/shared'
  ...
  (config) => {
    try {
      ...
      const isRefreshRequest =
        typeof config.url === 'string' && /\/accounts\/refresh(\b|\/|\?)/.test(config.url)
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status === 401 && !config.__retriedAfterRefresh) {
      try {
        ...
        if (!refreshingPromise) {
          refreshingPromise = authHandlers
            .refreshAccessToken()
            .catch((e) => {
              // 刷新失败，触发失败处理
              try {
    ...
    if (
      !skip &&
      !config.__retriedAfterRegistration &&
      typeof backendMessage === 'string' &&
      backendMessage.startsWith('设备不存在')
    ) {
      // 从 headers / url / body 提取 uuid
      const uuidFromHeader = getHeaderIgnoreCase(config.headers, HEADER_DEVICE_UUID)

...
import { DEFAULT_LOCAL_SERVER, HEADER_APP_TOKEN, HEADER_DEVICE_UUID } from '@classworks/shared'

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || DEFAULT_LOCAL_SERVER
...
class ApiClient {
  ...
  async revokeToken(targetToken, authOptions = {}) {
    ...
    if (usePathParam) {
      ...
      if (bearerToken) {
        ...
        headers[HEADER_DEVICE_UUID] = deviceUuid
      }
  ...
  async getTokensWithAuth(authType, authValue, options = {}) {
    ...
    switch (authType) {
      case 'uuid':
        headers[HEADER_DEVICE_UUID] = authValue
        params.set('uuid', authValue)
        break
      case 'token':
        headers[HEADER_APP_TOKEN] = authValue
        break
      case 'bearer':
  ...
  async getAutoAuthConfigs(deviceUuid, token) {
    return this.authenticatedFetch(
      `/auto-auth/devices/${deviceUuid}/auth-configs`,
      {
        method: 'GET',
      },
      token,
  ...
  async updateAutoAuthConfig(deviceUuid, token, configId, updates) {
    return this.authenticatedFetch(
      `/auto-auth/devices/${deviceUuid}/auth-configs/${configId}`,
      {
        method: 'PUT',
  ...
  async updateDeviceNamespace(deviceUuid, token, namespace) {
    return this.authenticatedFetch(
      `/auto-auth/devices/${deviceUuid}/namespace`,
      {
        method: 'PUT',

...
const ACCESS_TOKEN_SECRET =
  process.env.JWT_SECRET || 'your-access-token-secret-change-this-in-production'
const REFRESH_TOKEN_SECRET =
  process.env.REFRESH_TOKEN_SECRET || 'your-refresh-token-secret-change-this-in-production'
...
function getKeys(tokenType = 'access') {
  if (JWT_ALG === 'RS256') {
    ...
    if (!privateKey || !publicKey) {
      throw new Error(
        `RS256 需要同时提供 ${tokenType.toUpperCase()}_TOKEN_PRIVATE_KEY 与 ${tokenType.toUpperCase()}_TOKEN_PUBLIC_KEY`,
      )
    }

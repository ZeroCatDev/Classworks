...
const plan = await createImportPlan(importedDocument, [{ name: '数学', order: 0 }], async (date) =>
  clone(boardsByDate[date]),
)
assert.deepEqual(
  plan.rows.map((row) => [row.targetType, row.conflict, row.action]),
  [
    ['homework', true, 'keep'],
    ['custom', true, 'keep'],
    ['custom', false, 'import'],
...
duplicatePlan.rows.forEach((row) => {

...
export function createUafDocument(items, dateValue) {
  ...
  const assignments = items
    ...
    .map((item) => ({
      ...
      style: item.style
        ? JSON.parse(JSON.stringify(item.style))
        : JSON.parse(JSON.stringify(DEFAULT_ASSIGNMENT_STYLE)),
    }))
...
export function createExportPreview(items, dateValue) {
  ...
  return items
    ...
    .map((item, index) => {
      ...
      const issues = validateUafAssignment(
        assignment,
        assignment.subject || `第 ${index + 1} 张卡片`,
      )
      return {
        id: `${item.key || index}-${index}`,
        assignment,
